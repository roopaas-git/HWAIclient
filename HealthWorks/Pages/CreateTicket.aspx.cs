...
namespace HealthWorks.Pages
{
    public partial class CreateTicket : System.Web.UI.Page
    {
        ...
        {
            LinkButton lb = (LinkButton)Master.FindControl("lbCreateNew");
        ...
        {
            ddlCategory.DataSource = ticketDetailMethods.GetCategory();
            ddlCategory.DataValueField = "Cat_Name";
        ...
        public void Get_Sub_Category()
        {
            ddlSubCategory.DataSource = ticketDetailMethods.GetSubCategory(ddlCategory.SelectedItem.ToString());
            ddlSubCategory.DataTextField = "SubCat_Name";
            ddlSubCategory.DataValueField = "SubCat_Name";
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                int returnValue = ticketDetailMethods.insertTicket(Session["UserName"].ToString(), Session["FirstName"].ToString(), txtIssue.Text, ddlPriority.SelectedValue, "Open", DateTime.Today, ddlCategory.SelectedValue, ddlSubCategory.SelectedValue);
                if (returnValue > 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowStatus", "javascript:alert('Ticket is Raised');", true);
                    emailServices.SendTicketDetails(Session["FirstName"].ToString(), returnValue, ddlPriority.SelectedValue, "Ticket is Raised !", Session["UserName"].ToString(), txtIssue.Text.ToString(), ddlSubCategory.SelectedValue);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowStatus", "javascript:alert('Ticket is not Raised');", true);
                }
            ...
            try
            {
                foreach (GridViewRow row in Gvd_ViewTicket.Rows)
                {
                    if ((sender as LinkButton).ClientID == (row.FindControl("lnkSelect") as LinkButton).ClientID)
                    {
                        ...
                        txtCreatedate.Text = Date.ToString("MM/dd/yyyy");
        ...
        protected void btnReopen_Click(object sender, System.EventArgs e)
        {
            ...
            {
                int num = 0;
                int returnvalue = ticketDetailMethods.updateTicket(txtIssue1.Text, txtPriority.Text, ddlReopen.SelectedValue, Convert.ToInt32(txtTicketID.Text), num);

                if (returnvalue == 1)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowStatus", "javascript:alert('Ticket is Re-Opened');", true);
                    //emailServices.SendTicketDetails(Session["FirstName"].ToString(), returnvalue, ddlPriority.SelectedItem.Text, "Ticket is Re-Opened !");
                    ModalProgress.Hide();
                    Clear_Textboxes();
                }
                else
